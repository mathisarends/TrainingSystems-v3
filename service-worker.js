...
self.addEventListener("fetch", async event => {
  ...
  if (!defaultNetworkMode) {
    if (isCSS || isJS || isManfifest || isImage || isFont || isPage || isAudio) {
      ...
      (isPage && url.pathname.includes("/logout")) ||
      url.pathname.includes("/login") ||
      url.pathname.includes("/auth/google") ||
      (event.request.method === "POST" && event.request.url.includes("/login"))
    ) {
      response = event.respondWith(offlineFallBackPage(event));
      responseSet = true;
    } else {
      response = cacheOnly(event);
      responseSet = true;
  ...
  if (!responseSet) {
    if (isCSS || isJS || isManfifest || isImage || isFont) {
      ...
      /* defaultNetworkMode = true; */ // warum hier den network mode setzen
...
function offlineFallBackPage(ev) {
  return caches.match("/offline");
}
