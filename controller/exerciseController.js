...
export async function patchUserExercises(req, res) {
  try {
    ...
    const exerciseCategoriesLength = parseInt(req.body.exerciseCategoriesLength);
    ...
    for (let i = 0; i < exerciseCategoriesLength; i++) {
      exerciseCategoryPauseTimes.push(exerciseData[`categoryPauseTimeSelect_${i}`]);
      exerciseCategorySets.push(exerciseData[`categoryDefaultSetSelect_${i}`]);
      exerciseCategoryReps.push(exerciseData[`categoryDefaultRepSelect_${i}`]);
      exerciseCategoryRPE.push(exerciseData[`categoryDefaultRPESelect_${i}`]);
    }

    const numberOfRequestedExercises = getNumberOfRequestedExercises(
      exerciseCategoriesLength,
      maxAmountOfExercises,
      exerciseData
    );
    if (numberOfRequestedExercises === user.exercises.length) {
      //Die Anzahl der übungen sind gleich deswegen reichen partielle Änderungen:
      ...
      // es gibt mindestens eine neue Exercise und deswegen poste ich die einfach komplett nicht optimal aber...
...
function prepareExercisesData(user) {
  ...
  const exerciseCategories = [...new Set(predefinedExercises.map(exercise => exercise.category.name))];
